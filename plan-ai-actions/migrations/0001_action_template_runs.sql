...
-- YAML files baked into the host binary; this table holds one row per
-- execution, doubling as the work queue (status queued/running) and the
-- checkpoint store (next_step/variables/steps) so interrupted runs resume
-- after a restart without re-executing completed steps.
